...
<html>
    ...
    <body>
        ...
        <c:choose>
            ...
            <c:when test="${ empty applicationScope.clients }">
            ...
            <table>
                ...
                <c:forEach items="${ applicationScope.clients }" var="client" varStatus="boucle">
                <%-- Simple test de parité sur l'index de parcours, pour alterner la couleur de fond de chaque ligne du tableau. --%>
                <tr class="${boucle.index % 2 == 0 ? 'pair' : 'impair'}">
                    <%-- Affichage des propriétés du bean Client, qui est stocké en tant que valeur de l'entrée courante de la map --%>
                    <td>
                    	<a href="<c:url value="/afficherClient"><c:param name="idClient" value="${ client.key }" /></c:url>">
                    		<c:out value="${ client.value.id }"/>
                    	</a>
                    </td>
                    ...
                    <td class="action">
                        <a href="<c:url value="/suppressionClient"><c:param name="idClient" value="${ client.key }" /></c:url>">
